      ...
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMin slice" viewBox="0 0 728 400">
        ...
        <text font-family="monospace" font-size="10" fill="#1d1f20">
          <textPath id="text" xlink:href="#s">
            <template v-for="random in randoms" :key="random.id">
              {{ " " + random.value }} 
              <template v-for="breakdown in random.breakdowns" :key="breakdown.id">
                {{ breakdown.value }}
              </template>
            </template>
          </textPath>
...
export default {
  ...
  methods: {
    async generateRandom() {
      const response = await axios.post('http://127.0.0.1:8000/api/randoms')
      this.randoms = response.data.randoms
    },

    async getRandoms() {
      try {
        const response = await axios.get('http://127.0.0.1:8000/api/randoms')
        this.randoms = response.data.randoms
      } catch (error) {
